...
//**************************
// Ajout de la Fonction Reveil par Stéphane Gadeau (2019)
//**************************
String Ver = "3.4.1";
//**************************
//**************************
...
#define PINbouton2 2 // Bouton 2   -- GPIO2  ( d9 pour wemos r1 ou d4 pour wemos mini
...
#define Buzzer D2 // pin du buzzer (d2 pour wemos)
...
//#include <TimeAlarms.h> // ajouté pour la fonction reveil (moi) ne semble pas fonctionner
...
const size_t bufferSize = JSON_ARRAY_SIZE(6) + JSON_OBJECT_SIZE(3) + JSON_OBJECT_SIZE(7) + JSON_OBJECT_SIZE(10) + JSON_OBJECT_SIZE(29) + 800;
...
bool msgTemp;
bool raz;
bool reveil; // On - off du reveil (moi)
...
String JsonConfig(bool NC = false) {
  ...
  if (config.DEBUG) Serial.println("Mise a jour valeur Options : SEC=" + String(DisSec) + " HOR =" + String(TimeOn) + " LUM=" + String(AutoIn) + " REV =" + String(reveil));
  // Set the values
  ...
  rootcfg["reveil"] = reveil; // (moi)
  rootcfg["heureReveil"] = heureReveil; // (moi)
...
void loadConfiguration(const char *fileconfig, Config &config) {
  ...
  // Initialisation des variables systèmes pour configuration , si non présente valeur par defaut affecté
  ...
  reveil = rootcfg["reveil"] | false; // (moi)
  heureReveil = rootcfg["heureReveil"] | "10:30"; // (moi)
...
// fonction flashlight ( falsh LED avant notif ) (+ ajout du Buzzer supprimé)
void flashlight() {
  int F = 0;
  for (int i = 0; i < 8; i++) {
...
// fonction joue melodie
bool joueMelodie(const char* choixMelodie) {
  if ( !rtttl::isPlaying() ) {
    rtttl::begin(Buzzer, choixMelodie);
  } else {
    if (digitalRead(PINbouton1) == HIGH and digitalRead(PINbouton2) == HIGH and melodieStopped == false) {
      rtttl::play();
    } else {
      //     melodieStopped = true;
      rtttl::stop();
...
void handleNotif() {
  ...
  // on recupere les parametre dans l'url dans la partie /Notification?msg="notification a afficher"&type="PAC"
...
void handleOptions() {
  ...
  else if ( server.hasArg("HeureReveil")) {  //recupere l'heure du reveil (moi)
    String heureReveil = server.arg("HeureReveil");
  }
  else if ( server.hasArg("REV")) {  //active le reveil (moi)
    String REV = server.arg("REV");
    if (REV == "on" || REV == "1") Option(&reveil, true);
    else if (REV == "off" || REV == "0") Option(&reveil, false);
    else result = "idem";
  }
  ...
  //else if ( server.hasArg("INT") && !AutoIn) {
    ...
    if (minuteur > 0) cR = true;
      ...
      cR = false;
      NotifMsg("Annulation Minuteur", Intensite, false);
    }
    if (config.LED) ledOnOff(1);
    JSONSystem["CR"] = cR;
  }
    ...
    case WStype_TEXT:
      {
        ...
        if (config.DEBUG) Serial.println(" chaine payload : " + s);
        //envoi info systeme
        if ( s == "GetInfo") {
          String UptimeNotif;
          UptimeNotif = NTP.getUptimeString();
        ...
        // Reponse sauvegarde config
          ...
          else if (O == "REV") // (moi)
          {
            if (reveil != val ) {
              Option(&reveil, val);
              reponsetxt = "-Ok- Option du Reveil Mise à jour , Nouvelle Valeur : " + String(val);
              Serial.println("Heure Reveil = " + heureReveil); // moi a supprimer
            }
          }
          else if (O == "HeureReveil") { // (moi)
            heureReveil = jsoncfg["nouvelleHeureReveil"].as<String>();
            reponsetxt = "-Ok- Option du Reveil Mise à jour , Nouvelle Heure : " + heureReveil;
            Serial.println("Heure Reveil = " + heureReveil); // moi a supprimer
          }
...
// TimeOn , DisSec / AutoIn , Reveil
//**********************************

void Option(bool *Poption, bool etat) {
  // affectation de la valeur à l'option ( via pointeur )
  *Poption = etat;
  ...
  else if ( Poption == &reveil  )  { // (moi)
    JSONOptions["REV"] = reveil;
    JSONOptions["HeureReveil"] = heureReveil;
    if (etat)
    {
      NotifMsg("Reveil Actif à " + heureReveil, Intensite, true);
    }
  }
  ...
  if (config.DEBUG) {
    ...
    Serial.println("taille de update :" + String(i) + "valeur de Update " + Up);
  }
...
void setup(void)
{
  ...
  if (config.DEBUG)  {
    ...
    Serial.println("Reveil :" + String(reveil)); // (moi)
    Serial.println("HeureReveil :" + String(heureReveil)); // (moi)
  ...
  JSONOptions["txtAnim"] = txtAnim;
  JSONOptions["LEDstate"] = !ledState;
  JSONOptions["REV"] = reveil; // (moi)
  JSONOptions["HeureReveil"] = heureReveil; // (moi)
  ...
  if (config.DEBUG)  {
    ...
    Serial.print(" ,valeur affichage Secondes " + String(DisSec));
    Serial.print(" ,valeur Affichage horloge " + String(TimeOn));
    Serial.print(" ,valeur Affichage reveil " + String(reveil)); // (moi)
    Serial.print(" ,valeur Affichage heureReveil " + String(heureReveil)); // (moi)
    Serial.println("");
  }
...
void loop(void)
{
  ...
  //**************** Reveil ******************* (moi)
  time_t t = now();
  String hReveil, mReveil;
  hReveil = heureReveil.substring(0, 2);
  mReveil = heureReveil.substring(3);
  if ( reveil == 1 ) {
    if ( String(hour(t)) == hReveil && String(minute(t)) == mReveil ) {
      if (melodieStopped == false) {
        joueMelodie(mario);
      }
    } else {
      melodieStopped = false ;
    }
  }

  if (config.DEBUG) {
    Serial.println("===============================================");
    Serial.println("heureReveil = heure : " + hReveil + " minute : " + mReveil );
    Serial.println("Time Now = heure : " + String(hour(t)) + " minute : " + String(minute(t)) );
    Serial.println("Reveil actif : " + String(reveil));
    Serial.println("Condition : " + String(( String(hour(t)) == hReveil && String(minute(t)) == mReveil )));
  }
  if (P.displayAnimate())
  {

    if (Alert) {
      if (Fnotif && config.LED) {
        flashlight();
        while (joueMelodie(arkanoid) == false) {
          joueMelodie(arkanoid); // ici la melodie est celle choisie pour les notifications
        }
      }
    ...
    if (cR) {
      ...
      if (minuteur == 0) {
        ...
        if (config.CRLED && config.LED ) {
          flashlight(); // ledOnOff(0) remplacé par flashlight
          while (joueMelodie(tetris) == false) {
            joueMelodie(tetris); // ici la melodie est celle choisie pour le minuteur
          }
          //         joueMelodie(tetris); // ici la melodie est celle choisie pour le minuteur
          webSocket.broadcastTXT("Update");
        }
